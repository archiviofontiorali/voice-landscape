...
class ShareWidget {
  ...
  updatePreview () {
    const text = this.inputMessage.val().trim()
  ...
  enableSubmit () {
    if ($(this.inputMessage).val() &&
            ...
            $(this.inputLongitude).val()) { this.buttonSubmit.prop('disabled', false) }
  }
  ...
  getPlacePosition () {
    ...
    const coords = new Coordinates(option.data('latitude'), option.data('longitude'))
  ...
  getAutoPosition () {
    if (this.autoLocationIcon.attr('data-state') === 'processing') { return }
