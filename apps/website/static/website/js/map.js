...
class WordCloudMarker {
  constructor (index = 0, coordinates = [0.0, 0.0], options) {
    this.index = index
  ...
  addMarker (map) {
    ...
    this.marker = L.marker(this.coordinates, { icon }).addTo(map)
...
class LeafletMap {
  constructor (id = 'map', center = [0.0, 0.0], options) {
    this.center = center
  ...
  _addBackground () {
    ...
    if (name) { return L.tileLayer.provider(name, opts).addTo(this.map) } else { return L.tileLayer(url, opts).addTo(this.map) }
  }
  ...
  addWordClouds (places) {
    this.places = places
    const width = this.markerWidth; const height = this.markerHeight
