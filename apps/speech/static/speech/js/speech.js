...
class SpeechToText {
  ...
  async openStream () {
    if (!window.navigator || !window.navigator.mediaDevices) { throw new MediaDeviceNotAvailableError('navigator.mediaDevice not available') }
  ...
  handleResponse (text) {
    ...
    if (!this.output.val() ||
           confirm(`Trascrizione:\n\n${text}\n\nVuoi sostituirlo al messaggio attuale?`)) { this.output.val(text) }
  }
